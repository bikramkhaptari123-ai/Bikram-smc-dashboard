...
if asset_class == "NEPSE":
    is_nepse = True
    nepse_symbols = ["NABIL", "NMB", "NICA", "KBL", "GBIME", "EBL", "PCBL", "AKPL", "UPPER", "BHPL", "NRIC", "HRL", "NLIC", "CIT", "NTC"]
...
def fetch_global_data(ticker, tf):
    try:
        ...
        if tf == "15D" and interval == "1d":
            data = data.resample('15D').agg({'open': 'first', 'high': 'max', 'low': 'min', 'close': 'last', 'volume': 'sum'}).dropna()
...
if df is not None and not df.empty:
    ...
    fig = make_subplots(rows=3, cols=1, shared_xaxes=True, vertical_spacing=0.03, row_heights=[0.65, 0.17, 0.18])

    # Proper Candlestick chart with solid filled bodies
